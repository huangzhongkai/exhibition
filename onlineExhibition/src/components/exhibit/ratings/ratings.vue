...
//        }
      ...
      upload(){
        ...
        if(this.type === false){
          ...
          this.$http.post('http://'+ host +'/exhibit_ratings/?id='+ this.exhibit_id,rating, {emulateJSON:true} ).then(response => {
          ...
          this.$http.post('http://'+ host +'/exhibit_remark/?id='+ this.exhibit_id,this.remark, {emulateJSON:true} ).then(response => {
            if(response.body === 'error'){
              window.location = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx522cca3d4b048aa9&redirect_uri=http%3A//'+ encodeURIComponent(host) +'/home_html/&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect'
            }else{
              this.showFlag = false;
